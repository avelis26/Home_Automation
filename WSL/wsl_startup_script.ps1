...
$LogFile = "C:\Scripts\wsl-emby-startup.log"
...
try {
    ...
    $mountCheck = wsl -d Ubuntu-22.04 -u root test -d /mnt/wsl/PHYSICALDRIVE1p1 '&&' echo "mounted" '||' echo "not mounted"
    ...
    wsl -d Ubuntu-22.04 -u root -- systemctl start emby-server
    wsl -d Ubuntu-22.04 -u root -- systemctl start smbd
    wsl -d Ubuntu-22.04 -u root -- systemctl start transmission-daemon
    
    Write-Log "All services started successfully"
    
    # Display service status
    Write-Log "Service Status:"
    $embyStatus = wsl -d Ubuntu-22.04 -u root -- systemctl is-active emby-server
    $sambaStatus = wsl -d Ubuntu-22.04 -u root -- systemctl is-active smbd
    $transStatus = wsl -d Ubuntu-22.04 -u root -- systemctl is-active transmission-daemon
    ...
    $wslIP = wsl -d Ubuntu-22.04 hostname -I | ForEach-Object { $_.Trim() }
    Write-Log ""
    Write-Log "Access Emby at: http://$wslIP`:8096"
    Write-Log "Access Samba share at: \\wsl$\Ubuntu-22.04\mnt\wsl\PHYSICALDRIVE1p1"
